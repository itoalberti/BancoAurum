...
export default class UsuarioDAO {
    ...
    if (usuarioProduto instanceof UsuarioProduto) {
      ...
      try {
        const sql = `INSERT INTO Usuario_Produto (usu_codigo, pro_codigo, up_data_contratacao, up_saldo, up_valor_final)
                             VALUES (?,?,?,?,?)`;
        const parametros = [usuarioProduto.usuario.codigo, usuarioProduto.produto.codigo, usuarioProduto.data_contratacao, usuarioProduto.saldo, usuarioProduto.valor_final];
  ...
  async consultar(termo) {
    ...
    if (!isNaN(parseInt(termo)) && registros.length > 0) {
      const agencia = new Agencia(registros[0].ag_codigo, registros[0].ag_numero, registros[0].ag_telefone, registros[0].ag_email, registros[0].ag_cep, registros[0].ag_endereco, registros[0].ag_cidade, registros[0].ag_uf);
      const usuario = new Usuario(
        registros[0].usu_codigo,
        registros[0].usu_tipo,
        registros[0].usu_nome,
        registros[0].usu_cpf,
        registros[0].usu_rg,
        registros[0].usu_genero,
        registros[0].usu_telefone,
        registros[0].usu_data_nascimento,
        registros[0].usu_cep,
        registros[0].usu_endereco,
        registros[0].usu_cidade,
        registros[0].usu_uf,
        registros[0].usu_email,
        registros[0].usu_senha,
        agencia
      );
      ...
      for (const registro of registros) {
        const agencia = new Agencia(registro.ag_codigo, registro.ag_numero, registro.ag_telefone, registro.ag_email, registro.ag_cep, registro.ag_endereco, registro.ag_cidade, registro.ag_uf);
        ...
        const usuario = new Usuario(
          ...
          registro.usu_rg,
          registro.usu_genero,
          registro.usu_telefone,
          registro.usu_data_nascimento,
          registro.usu_cep,
          registro.usu_endereco,
          registro.usu_cidade,
          registro.usu_uf,
          registro.usu_email,
          registro.usu_senha,
    ...
    conexao.release();
    if (!isNaN(parseInt(termo))) return listaProdutos;
    ...
    if (usuario instanceof Usuario) {
      ...
      try {
        const sql = `UPDATE Usuario SET usu_tipo = ?, usu_telefone = ?, usu_cep = ?, usu_endereco = ?,
                             ...
                             WHERE usu_codigo = ?`;
        const parametros = [usuario.tipo, usuario.telefone, usuario.cep, usuario.endereco, usuario.cidade, usuario.uf, usuario.email, usuario.senha, usuario.agencia.codigo, usuario.codigo];
        await conexao.execute(sql, parametros);
        await conexao.commit();
      } catch (erro) {
        await conexao.rollback();
  ...
  async alterarProduto(usuarioProduto) {
    if (usuarioProduto instanceof UsuarioProduto) {
      ...
      try {
        ...
        const parametros = [usuarioProduto.data_contratacao, usuarioProduto.saldo, usuarioProduto.usuario.codigo, usuarioProduto.produto.codigo];
        await conexao.execute(sql, parametros);
        await conexao.commit();
      } catch (erro) {
        await conexao.rollback();
