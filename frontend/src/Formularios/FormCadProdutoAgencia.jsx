...
export default function FormCadProdutoAgencia(props) {
  ...
  function manipularMudanca(evento) {
    ...
    if (componente.name === 'agencia') setProdutoAgencia({ ...produtoAgencia, agencia: { codigo: componente.value } });
    else if (componente.name === 'produto') setProdutoAgencia({ ...produtoAgencia, produto: { codigo: componente.value } });
    else setProdutoAgencia({ ...produtoAgencia, [componente.name]: componente.value });
  }
  ...
  function manipularSubmissao(evento) {
    ...
    if (!form.checkValidity()) setValidado(false);
      ...
      setValidado(true);
      if (!props.atualizando) props.gravarProdutoAgencia(produtoAgencia);
      else props.alterarProdutoAgencia(produtoAgencia);
    }
  }
    ...
    <Form noValidate validated={!validado} onSubmit={manipularSubmissao}>
      <Row className='mb-3'>
        ...
        <Form.Group as={Col} md='3'>
          ...
          <Form.Select disabled value={produtoAgencia.agencia.codigo} id='agencia' name='agencia' onChange={manipularMudanca}>
            <option key={produtoAgencia.agencia.codigo} value={produtoAgencia.agencia.codigo}>
              ({produtoAgencia.agencia.numero}) - {produtoAgencia.agencia.cidade}
            </option>
          </Form.Select>
        </Form.Group>
        ...
        <Form.Group as={Col} md='3'>
          ...
          <Form.Select disabled={props.atualizando} required={!props.atualizando} value={produtoAgencia.produto.codigo} id='produto' name='produto' onChange={manipularMudanca}>
      ...
      <Row className='mb-3'>
        ...
        <Form.Group as={Col} md='2'>
          ...
          <Form.Control required type='date' placeholder='' value={produtoAgencia.data_adesao} id='data_adesao' name='data_adesao' onChange={manipularMudanca} />
