...
export default function FormCadProdutoUsuario(props) {
  ...
  function manipularMudanca(evento) {
    ...
    if (componente.name === 'usuario') setProdutoUsuario({ ...produtoUsuario, usuario: { codigo: componente.value } });
      ...
      if (componente.value !== '') {
        if (produtoSelecionado.produto.tipo === 'Empréstimo' || produtoSelecionado.produto.tipo === 'Financiamento' || produtoSelecionado.produto.tipo === 'Seguro') {
          let valorFinal;
          if (produtoSelecionado.produto.tipo !== 'Seguro') valorFinal = parseFloat(produtoSelecionado.produto.valor + produtoSelecionado.produto.valor * (produtoSelecionado.produto.juros / 100).toFixed(2));
          else valorFinal = produtoSelecionado.produto.valor;
          setProdutoUsuario({ ...produtoUsuario, produto: produtoSelecionado.produto, valor_final: valorFinal });
        } else setProdutoUsuario({ ...produtoUsuario, produto: produtoSelecionado.produto });
      } else setProdutoUsuario({ ...produtoUsuario, produto: { codigo: componente.value } });
    } else setProdutoUsuario({ ...produtoUsuario, [componente.name]: componente.value });
  }
  ...
  function manipularSubmissao(evento) {
    ...
    if (!form.checkValidity()) setValidado(false);
      ...
      setValidado(true);
      if (!props.atualizando) props.gravarProdutoUsuario(produtoUsuario);
      else props.alterarProdutoUsuario(produtoUsuario);
    }
  }
    ...
    <Form noValidate validated={!validado} onSubmit={manipularSubmissao}>
      <Row className='mb-3'>
        ...
        <Form.Group as={Col} md='3'>
          ...
          <Form.Select disabled value={produtoUsuario.usuario.codigo} id='usuario' name='usuario' onChange={manipularMudanca}>
            <option key={produtoUsuario.usuario.codigo} value={produtoUsuario.usuario.codigo}>
              {produtoUsuario.usuario.nome}
            </option>
          </Form.Select>
        </Form.Group>
        ...
        <Form.Group as={Col} md='3'>
          ...
          <Form.Select disabled={props.atualizando} required={!props.atualizando} value={produtoUsuario.produto.codigo} id='produto' name='produto' onChange={manipularMudanca}>
            {props.listaProdutos[0].produto.codigo !== 0 ? (
              <>
                ...
                {props.listaProdutos.map((produtoUsuario) => {
                  return (
                    <option key={produtoUsuario.produto.codigo} value={produtoUsuario.produto.codigo}>
                      ({produtoUsuario.produto.tipo}) - {produtoUsuario.produto.nome}
                    </option>
                  );
                })}
              </>
              ...
              <option key={0} value={''}>
                {props.listaProdutos[0].produto.nome}
              </option>
            )}
          </Form.Select>
      ...
      <Row className='mb-3'>
        ...
        <Form.Group as={Col} md='2'>
          ...
          <Form.Control required type='date' placeholder='' value={produtoUsuario.data_contratacao} id='data_contratacao' name='data_contratacao' onChange={manipularMudanca} />
        ...
        {produtoUsuario.produto.tipo === 'Cartão de Crédito' && (
          <Form.Group as={Col} md='2'>
            ...
            <Form.Control required type='number' placeholder='Saldo' value={produtoUsuario.saldo} id='saldo' name='saldo' onChange={manipularMudanca} />
            <Form.Control.Feedback type='invalid'>Por favor, informe o saldo!</Form.Control.Feedback>
          </Form.Group>
        )}
        {(produtoUsuario.produto.tipo === 'Empréstimo' || produtoUsuario.produto.tipo === 'Financiamento' || produtoUsuario.produto.tipo === 'Seguro') && (
          <Form.Group as={Col} md='2'>
            {/* Valor Final do Produto com Juros */}
            <Form.Label>Valor Final</Form.Label>
            <Form.Control disabled type='number' placeholder='Valor Final' value={produtoUsuario.valor_final} id='valor_final' name='valor_final' onChange={manipularMudanca} />
