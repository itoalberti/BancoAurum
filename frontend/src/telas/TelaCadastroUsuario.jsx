...
export default function TelaCadastroUsuario(props) {
  ...
  async function consultarAgencia() {
    await fetch(urlAgencia, { method: 'GET' })
      ...
      .then((retorno) => {
        if (retorno.status) setListaAgencias(retorno.listaAgencias);
  ...
  async function consultarUsuario() {
    await fetch(urlUsuario, { method: 'GET' })
      .then((resposta) => resposta.json())
      .then((retorno) => {
        if (retorno.status) setListaUsuarios(retorno.listaUsuarios);
        else alert(retorno.mensagem);
      })
      .catch((erro) => {
        alert('Erro: ' + erro.mensagem);
  ...
  useEffect(() => {
    if (exibirTabela) consultarUsuario();
    else consultarAgencia();
  }, [exibirTabela]);
  ...
  async function excluirUsuario(usuario) {
    await fetch(urlUsuario, {
      ...
      .then((resposta) => resposta.json())
      .then((retorno) => {
        if (retorno.status) alert(retorno.mensagem);
        else alert(retorno.mensagem);
      })
      .catch((erro) => {
        alert('Erro: ' + erro.message);
  ...
  if (exibirTabela) {
    ...
    return (
      <Pagina titulo='Cadastro de Usuários'>
        ...
        <TabelaUsuario setExibirTabela={setExibirTabela} listaUsuarios={listaUsuarios} alterarUsuario={alterarUsuario} excluirUsuario={excluirUsuario} />
      </Pagina>
    );
  } // Se a variável exibirTabela for falsa, exibe o formulário de cadastro de usuários
  else {
    return (
      <Pagina titulo='Cadastro de Usuários'>
