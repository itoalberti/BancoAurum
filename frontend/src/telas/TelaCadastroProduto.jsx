...
export default function TelaCadastrarProduto(props) {
  ...
  async function consultarProduto() {
    await fetch(urlProduto, { method: 'GET' })
      ...
      .then((retorno) => {
        if (retorno.status) setListaProdutos(retorno.listaProdutos);
  ...
  useEffect(() => {
    if (exibirTabela) consultarProduto();
  }, [exibirTabela]);
    ...
    return (
      <div>
        <Pagina>
          ...
          <TabelaProduto setExibirTabela={setExibirTabela} listaProdutos={listaProdutos} alterarProduto={alterarProduto} excluirProduto={excluirProduto} />
        </Pagina>
      </div>
    );
